const Blood = artifacts.require("./src/contracts/Blood.sol");
...
contract('Blood', ([donor, bank, hosp, admin, bank2, hosp2]) => {
  ...
  describe('Bloodbags', async () => {
    ...
    before(async () => {
      ...
      await blood.createBloodbag(donation, donor, "Donor A", "1234567890","A+", expiry, "Mumbai", { from: bank2 })
      await blood.createBloodbag(donation, donor, "Donor B", "1234567890", "AB-", expiry, "Mumbai", { from: bank2 })
      await blood.createBloodbag(donation, donor, "Donor C", "1234567890", "O-", expiry, "Mumbai", { from: bank })
      await blood.createBloodbag(donation, donor, "Donor D", "1234567890", "AB-", expiry, "Mumbai", { from: bank2 })

      result = await blood.createBloodbag(donation, donor, "Chinmay", "1234567890", "A+", expiry, "Mumbai", { from: bank })
